...
class Genre:
    ...
    def get_final_rating(self):
        ...
        average_bpm = self.total_bpm / l
        average_pos= self.total_pos / l
        average_nrgy = self.total_nrgy / l

        final_rating = (average_rating ** 2) / (average_rating * 1.1 - 1)
        final_rating += (1 if Genre.bpmBool else -1) * (average_bpm-115) / 50
        final_rating += (1 if Genre.posBool else -1) * (average_pos-70) / 50
        final_rating += (1 if Genre.nrgyBool else -1) * (average_nrgy-77) / 50
        return final_rating
    ...
    def sort_by_rating(g):
        return g.get_final_rating()

    def __repr__(self):
        return f"\n{self.genre}-{self.get_final_rating()}"
